...
*/
  ...
  $authuserfile_array = array('##### ProjectShopping
 ADMIN PROTECTION - BEGIN #####',
                              'AuthType Basic',
                              'AuthName "ProjectShopping
 Online Merchant Administration Tool"',
                              'AuthUserFile ' . DIR_FS_ADMIN . '.htpasswd_ProjectShopping
',
                              'Require valid-user',
                              '##### ProjectShopping
 ADMIN PROTECTION - END #####');

  if (!$is_iis && file_exists(DIR_FS_ADMIN . '.htpasswd_ProjectShopping
') && tep_is_writable(DIR_FS_ADMIN . '.htpasswd_ProjectShopping
') && file_exists(DIR_FS_ADMIN . '.htaccess') && tep_is_writable(DIR_FS_ADMIN . '.htaccess')) {
    ...
    if (filesize(DIR_FS_ADMIN . '.htpasswd_ProjectShopping
') > 0) {
      $fg = fopen(DIR_FS_ADMIN . '.htpasswd_ProjectShopping
', 'rb');
      $data = fread($fg, filesize(DIR_FS_ADMIN . '.htpasswd_ProjectShopping
'));
      fclose($fg);
      ...
      case 'insert':
        ...
        if (tep_db_num_rows($check_query) < 1) {
          ...
          if (is_array($htpasswd_array)) {
            ...
            $fp = fopen(DIR_FS_ADMIN . '.htpasswd_ProjectShopping
', 'w');
            fwrite($fp, implode("\n", $htpasswd_array));
            fclose($fp);

            if (!in_array('AuthUserFile ' . DIR_FS_ADMIN . '.htpasswd_ProjectShopping
