...
// class constructor
    function moneybookers_idl() {
      ...
      $this->public_title = MODULE_PAYMENT_MONEYBOOKERS_IDL_TEXT_PUBLIC_TITLE;
      $this->description = MODULE_PAYMENT_MONEYBOOKERS_IDL_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_PAYMENT_MONEYBOOKERS_IDL_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_MONEYBOOKERS_IDL_STATUS == 'True') ? true : false);

      if ((int)MODULE_PAYMENT_MONEYBOOKERS_IDL_PREPARE_ORDER_STATUS_ID > 0) {
        $this->order_status = MODULE_PAYMENT_MONEYBOOKERS_IDL_PREPARE_ORDER_STATUS_ID;
      }
...
// class methods
    function update_status() {
      ...
      if (!defined('MODULE_PAYMENT_MONEYBOOKERS_STATUS')) {
        $this->enabled = false;
      } elseif ( ($this->enabled == true) && ((int)MODULE_PAYMENT_MONEYBOOKERS_IDL_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_MONEYBOOKERS_IDL_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_MONEYBOOKERS_IDL_STATUS'");
    ...
    function keys() {
      return array('MODULE_PAYMENT_MONEYBOOKERS_IDL_STATUS', 'MODULE_PAYMENT_MONEYBOOKERS_IDL_ZONE', 'MODULE_PAYMENT_MONEYBOOKERS_IDL_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_IDL_TRANSACTIONS_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_IDL_ORDER_STATUS_ID', 'MODULE_PAYMENT_MONEYBOOKERS_IDL_SORT_ORDER');
    }
  }
?>
